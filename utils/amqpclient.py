...
class AMQPQueue(object):
    ...
    def basic_publish(self, body, properties={}):
        ...
        self.channel.basic_publish(
            exchange=self.exchange, routing_key=self.routing_key, msg=msg)
...
if __name__ == "__main__":
    client = AMQPClient(host='127.0.0.1', userid='driver',
                        password='P@55word', virtual_host='/driver')
    # client.basic_qos(prefetch_size = 1)

    begin = time.time() * 1000
    producer = client.queue(
        'test_queue', 'test_exchange', 'direct', 'test_key')
    end = time.time() * 1000
