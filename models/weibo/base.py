    ...
    def _import_cassandra(self, keyspace, column_family):
        ...
        if dup:
            ...
            if not self.equals(dup):
                ...
                return True
                _LOGGER.info("UPDATED CASSANDRA MODEL - %s", self)
            else:
                _LOGGER.info("SKIPPED CASSANDRA MODEL - %s", self)
                return False
        else:
            self.save_cassandra(keyspace, column_family)
            _LOGGER.info("INSERTED CASSANDRA MODEL - %s", self)
            return True
    ...
    def _import_redis(self, name):
        ...
        if dup:
            ...
            if not self.equals(dup):
                ...
                return True
                _LOGGER.info("UPDATED REDIS MODEL - %s", self)
            else:
                _LOGGER.info("SKIPPED REDIS MODEL - %s", self)
                return False
        else:
            self.save_redis(name)
            _LOGGER.info("INSERTED REDIS MODEL - %s", self)
            return True

    def save_redis(self, name):
        self["id"] = self.get("id") if id else self.new_id()
        self["id"] = str(self["id"])
        # RedisQueryApi().set(name, self["id"], dict(self))
        # RedisQueryApi().expireat(self["id"], self['expire'])
        RedisQueryApi().SADD(name, self["id"])
        name = name + self["id"]
        RedisQueryApi().HMSET(name, dict(self))
        return self
